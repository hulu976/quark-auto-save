...
@app.route("/test_plugin", methods=["POST"])
def test_plugin():
    if not is_login():
        return jsonify({"success": False, "message": "未登录"})
    try:
        data = request.json
        plugin_name = data.get("plugin_name") or data.get("pluginName")
        plugin_config = data.get("plugin")
        
        if not plugin_name or not plugin_config:
            return jsonify({"success": False, "message": "缺少插件名称或配置"})
            
        log_lines = []
        log_lines.append(f"=== 测试 {plugin_name} ===")
        log_lines.append(f"时间：{time.strftime('%Y-%m-%d %H:%M:%S')}")
        log_lines.append("")
        
        if plugin_name == "fnv_refresh_v2":
            log_lines.append("正在测试 fnv_refresh_v2 插件...")
            endpoint = plugin_config.get('endpoint', '')
            if not endpoint.startswith('http'):
                endpoint = f"http://{endpoint}"
            username = plugin_config.get('username', '')
            password = plugin_config.get('password', '')
            mount_quark = plugin_config.get('mount_quark', '')
            media_name = plugin_config.get('media_name', '')
            remove_useless_wait = plugin_config.get('remove_useless_wait', '180')
            
            log_lines.append(f"endpoint: `{endpoint}`")
            log_lines.append(f"username: {username if username else '(空)'}")
            log_lines.append(f"password: {'***' if password else '(空)'}")
            log_lines.append(f"mount_quark: {mount_quark if mount_quark else '(空)'}")
            log_lines.append(f"media_name: {media_name if media_name else '(空)'}")
            log_lines.append(f"remove_useless_wait: {remove_useless_wait}")
            log_lines.append("")
            
            if username and password:
                try:
                    login_url = f"{endpoint}/v/api/v1/login"
                    log_lines.append(f"正在连接 `{login_url}`...")
                    ...
                    if response.status_code == 200:
                        ...
                        if result.get("code") == 0:
                            ...
                            log_lines.append(f"✅ 登录成功！Token: {token[:20]}...")
                            log_lines.append("")
                            
                            log_lines.append("=== 尝试刷新媒体库 ===")
                            try:
                                import importlib
                                mod = importlib.import_module("plugins.fnv_refresh_v2")
                                
                                if hasattr(mod, 'FnvClient'):
                                    FnvClient = getattr(mod, 'FnvClient')
                                    log_lines.append("✅ 成功加载 FnvClient")
                                    
                                    try:
                                        client = FnvClient(base_url=endpoint)
                                        client.set_fnv_token(token)
                                        log_lines.append("✅ FnvClient 初始化成功")
                                        
                                        log_lines.append("\n=== 获取系统信息 ===")
                                        sys_info = client.get_sys_info()
                                        if sys_info.get("code") == 0:
                                            sys_data = sys_info.get("data", {})
                                            log_lines.append(f"✅ 系统名称: {sys_data.get('name', '未知')}")
                                            log_lines.append(f"✅ 系统 GUID: {sys_data.get('guid', '未知')}")
                                        else:
                                            log_lines.append(f"❌ 获取系统信息失败: {sys_info.get('msg', '未知错误')}")
                                        
                                        log_lines.append("\n=== 获取媒体库列表 ===")
                                        media_libs = client.get_media_libraries()
                                        if media_libs.get("code") == 0:
                                            media_list = media_libs.get("data", [])
                                            log_lines.append(f"✅ 共找到 {len(media_list)} 个媒体库")
                                            for media in media_list:
                                                log_lines.append(f"  - {media.get('name')} (guid: {media.get('guid')[:10]}...)")
                                            ...
                                            for media in media_list:
                                                if media.get("name") == media_name:
                                                    ...
                                                    log_lines.append(f"\n✅ 找到媒体库 '{media_name}':")
                                                    log_lines.append(f"   guid: {target_guid}")
                                                    log_lines.append(f"   dir_list: {media.get('dir_list')}")
                                                    break
                                            
                                            if target_guid:
                                                log_lines.append(f"\n=== 扫描媒体库 '{media_name}' ===")
                                                scan_result = client.scan_media_library(target_guid, [mount_quark] if mount_quark else [])
                                                if scan_result.get("code") == 0:
                                                    log_lines.append(f"✅ 扫描成功！{scan_result.get('msg', '')}")
                                                else:
                                                    log_lines.append(f"⚠️ 扫描返回: {scan_result.get('msg', '')} (code: {scan_result.get('code')})")
                                            else:
                                                log_lines.append(f"\n⚠️ 未找到媒体库 '{media_name}'")
                                                log_lines.append("   将尝试使用第一个媒体库进行测试")
                                                if media_list:
                                                    first_guid = media_list[0].get("guid")
                                                    first_name = media_list[0].get("name")
                                                    log_lines.append(f"\n=== 扫描媒体库 '{first_name}' ===")
                                                    scan_result = client.scan_media_library(first_guid, [mount_quark] if mount_quark else [])
                                                    if scan_result.get("code") == 0:
                                                        log_lines.append(f"✅ 扫描成功！")
                                                    else:
                                                        log_lines.append(f"⚠️ 扫描返回: {scan_result.get('msg', '')}")
                                        else:
                                            log_lines.append(f"❌ 获取媒体库列表失败: {media_libs.get('msg', '未知错误')}")
                                        
                                    except Exception as client_err:
                                        log_lines.append(f"❌ FnvClient 使用失败: {client_err}")
                                        import traceback
                                        log_lines.append(f"详细信息:\n{traceback.format_exc()}")
                                else:
                                    log_lines.append("⚠️ FnvClient 类不存在，尝试加载插件主类...")
                                    try:
                                        PluginClass = getattr(mod, "Fnv_refresh_v2")
                                        plugin_instance = PluginClass(**plugin_config)
                                        log_lines.append("✅ 插件加载成功")
                                        if plugin_instance.is_active:
                                            log_lines.append("✅ 插件已激活")
                                            mock_task = {
                                                "addition": {
                                                    "fnv_refresh_v2": {
                                                        "auto_refresh": True,
                                                        "mdb_name": media_name or "影视"
                                                    }
                                                }
                                            }
                                            log_lines.append(f"正在刷新媒体库: {media_name or '影视'}...")
                                            plugin_instance.run(mock_task)
                                            log_lines.append("✅ 媒体库刷新指令已发送")
                                        else:
                                            log_lines.append("❌ 插件未激活，请检查配置")
                                    except Exception as load_err:
                                        log_lines.append(f"⚠️ 无法加载 fnv_refresh_v2 插件: {load_err}")
                                        log_lines.append("  （Windows 上 .so 文件无法运行，Linux/Docker 环境中可正常刷新）")
                            
                            except Exception as import_err:
                                log_lines.append(f"⚠️ 无法导入 fnv_refresh_v2 插件: {import_err}")
                                log_lines.append("  （Windows 上 .so 文件无法运行，Linux/Docker 环境中可正常刷新）")
                            
                        else:
                            log_lines.append(f"❌ 登录失败：{result.get('msg', '未知错误')}")
                    else:
                        log_lines.append(f"❌ HTTP 请求失败：{response.status_code}")
                except requests.exceptions.ConnectionError:
                    log_lines.append(f"❌ 无法连接到 {endpoint}")
                    log_lines.append("")
                    log_lines.append("请检查:")
                    log_lines.append("  1. 飞牛 NAS 是否开机并正常运行")
                    log_lines.append("  2. endpoint 地址是否正确 (格式：http://IP:端口)")
                    log_lines.append("  3. 飞牛影视是否已安装并启用")
                    log_lines.append("  4. 防火墙是否阻止了该端口")
                except requests.exceptions.Timeout:
                    log_lines.append(f"❌ 连接超时 ({endpoint})")
                    log_lines.append("请检查网络连通性")
                except Exception as e:
                    log_lines.append(f"❌ 错误：{str(e)}")
                    import traceback
                    log_lines.append(f"详细信息:\n{traceback.format_exc()}")
            else:
                log_lines.append("⚠️ 请输入用户名和密码进行测试")
        
        else:
            log_lines.append(f"不支持的插件类型：{plugin_name}")
        
        log_lines.insert(0, "=" * 60)
        log_lines.append("=" * 60)
        
        return jsonify({"success": True, "log": "\n".join(log_lines)})
    except Exception as e:
        import traceback
        error_log = f"=== 测试 {plugin_name} ===\n"
        error_log += f"❌ 异常：{str(e)}\n"
        error_log += f"详细信息:\n{traceback.format_exc()}"
        return jsonify({"success": False, "message": str(e), "log": error_log})
...
def init():
    ...
    # fnv_refresh_v2 默认配置（含 media_name 等字段）
    fnv_default_config = {
        "endpoint": "172.17.0.1:5666",
        "username": "",
        "password": "",
        "mount_quark": "",
        "media_name": "影视",
    ...
    if "fnv_refresh_v2" not in plugins_config_default:
        plugins_config_default["fnv_refresh_v2"] = fnv_default_config
    else:
        for key, value in fnv_default_config.items():
            if key not in plugins_config_default["fnv_refresh_v2"]:
                plugins_config_default["fnv_refresh_v2"][key] = value
