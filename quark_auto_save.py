...
def do_save(account, tasklist=[]):
    ...
    for plugin_name, plugin in plugins.items():
        if plugin.is_active and hasattr(plugin, "task_before"):
            for call_args in [dict(tasklist=tasklist, account=account), dict()]:
                try:
                    tasklist = plugin.task_before(**call_args) or tasklist
                    break
                except TypeError:
                    continue
                except Exception as e:
                    print(f"插件 {plugin_name} task_before 执行失败: {e}")
                    break
    ...
    for index, task in enumerate(tasklist):
        ...
        if not is_time(task):
            ...
            # 将全局 media_name 复制到 fnv_refresh_v2 的 mdb_name
            if "fnv_refresh_v2" in task["addition"]:
                media_name = CONFIG_DATA["plugins"].get("fnv_refresh_v2", {}).get("media_name")
                if media_name:
                    task["addition"]["fnv_refresh_v2"]["mdb_name"] = media_name
            # 调用插件
            if is_new_tree:
                print(f"🧩 调用插件")
                for plugin_name, plugin in plugins.items():
    ...
    for plugin_name, plugin in plugins.items():
        if plugin.is_active and hasattr(plugin, "task_after"):
            ...
            for call_args in [dict(tasklist=tasklist, account=account), dict()]:
                try:
                    data = plugin.task_after(**call_args)
                    break
                except TypeError:
                    continue
                except Exception as e:
                    print(f"插件 {plugin_name} task_after 执行失败: {e}")
                    break
